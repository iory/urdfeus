...
class TestConsoleScripts(unittest.TestCase):
    ...
    def test_mesh2eus(self):
        target_mesh = osp.join(osp.dirname(self.urdfpath), 'meshes',
                               'base_link.dae')
        output_eus_path = osp.join(osp.dirname(self.urdfpath), 'meshes',
                                   'base_link.l')
    ...
    def test_urdf2eus(self):
        output_eus_path = osp.join(
            osp.dirname(self.urdfpath), 'fetch.l')
